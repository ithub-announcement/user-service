...
class UsersController(private var usersService: UsersService) {
  ...
  @GetMapping("/profile")
  fun getUserByAccess(request: HttpServletRequest): ResponseEntity<Optional<User>> {
    return this.usersService.findByAccess(request.getHeader("Authorization"))
  }

  @GetMapping("/profile/{uid}")
  fun getUserByUID(@PathVariable uid: String): ResponseEntity<Optional<User>> {
    return this.usersService.findByUID(uid)
  }

  @PutMapping("/profile/{uid}/role/change")
  fun changeRole(@PathVariable uid: String, @RequestBody body: ChangeRolePayload, request: HttpServletRequest): ResponseEntity<User> {
    return this.usersService.changeRole(uid, body.role, request)
  }

  @PutMapping("/profile/{uid}/name/change")
  fun changeName(@PathVariable uid: String, @RequestBody body: ChangeNamePayload, request: HttpServletRequest): ResponseEntity<User> {
